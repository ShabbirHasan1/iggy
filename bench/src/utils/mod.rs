use iggy::{
    client::{Client, ConsumerGroupClient, SystemClient, UserClient},
    clients::{builder::IggyClientBuilder, client::IggyClient},
    consumer::Consumer,
    error::IggyError,
...
pub async fn create_consumer(
    ...
    match consumer_group_id {
        Some(consumer_group_id) => {
            client
                .join_consumer_group(stream_id, topic_id, &consumer_group_id.try_into().unwrap())
                .await
                .expect("Failed to join consumer group");
            Consumer::group(consumer_group_id.try_into().unwrap())
        }
...
fn message_batches_from_metrics(individual_metrics: &[BenchmarkIndividualMetrics]) -> Option<u32> {
    let batches: u64 = individual_metrics
        .iter()
...
pub fn params_from_args_and_metrics(
    ...
    let message_batches = if let Some(message_batches) = message_batches_from_metrics(metrics) {
        message_batches
    } else {
        args.message_batches().unwrap().get()
    };
...
fn recreate_bench_command(args: &IggyBenchArgs) -> String {
    ...
    let total_messages_size = args.total_messages_size();
