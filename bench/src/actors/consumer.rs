...
use crate::utils::rate_limiter::ByteBasedRateLimiter;
use crate::utils::{batch_total_size_bytes, batch_user_size_bytes, create_consumer};
...
impl Consumer {
    ...
    pub async fn run(&self) -> Result<BenchmarkIndividualMetrics, IggyError> {
        ...
        let rate_limiter = self.limit_bytes_per_second.map(ByteBasedRateLimiter::new);
        ...
        loop {
            ...
            let last_batch_user_size_bytes = batch_user_size_bytes(&polled_messages);
            ...
            self.finish_condition.account(last_batch_user_size_bytes);
