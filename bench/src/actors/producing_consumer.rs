...
use crate::utils::create_consumer;
    ...
    pub async fn run(&self) -> Result<BenchmarkIndividualMetrics, IggyError> {
        ...
        loop {
            ...
            self.finish_condition
                .account(received_batch_user_size_bytes);
        }

        info!(
            "ProducingConsumer #{} → sent and received {} messages ({} sent + {} received) in {} total batches of {}",
            self.actor_id,
            messages_processed.human_count_bytes(),
            (messages_processed/2).human_count_bytes(),
            (messages_processed/2).human_count_bytes(),
            batches_processed,
            bytes_processed.human_count_bytes()
        );
