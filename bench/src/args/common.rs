...
pub struct IggyBenchArgs {
    ...
    pub total_messages_size: Option<IggyByteSize>,
...
impl IggyBenchArgs {
    ...
    pub fn validate(&mut self) {
        ...
        if let (None, None) = (self.message_batches, self.total_messages_size) {
            self.message_batches = Some(DEFAULT_MESSAGE_BATCHES);
        }

        if let Some(total_messages_size) = self.total_messages_size {
            let samples = total_messages_size.as_bytes_u64() / self.message_size().min() as u64;
            if samples <= 1 {
                IggyBenchArgs::command()
                    .error(
                        ErrorKind::ArgumentConflict,
    ...
    pub fn total_messages_size(&self) -> Option<IggyByteSize> {
    ...
    pub fn generate_dir_name(&self) -> String {
        ...
        let data_volume_arg = match (self.total_messages_size, self.message_batches) {
